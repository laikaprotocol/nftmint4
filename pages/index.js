...
//
  ...
  async function callContractData(wallet) {
    ...
    const bananaPrice = await bananaContract.methods.rolePrice().call() 
  ...
  async function mintBanana(how_many_bananas) {
    if (bananaContract) {
      ...
      const gasAmount = await bananaContract.methods.mintRole(how_many_bananas).estimateGas({from: walletAddress, value: price})
      console.log("estimated gas",gasAmount)

      console.log({from: walletAddress, value: price})

      bananaContract.methods
            .mintRole(how_many_bananas)
  ...
  async function claimWiseToken(how_many_bananas) {
    if (bananaContract) {
      ...
      const gasAmount = await bananaContract.methods.redeemRole(how_many_bananas).estimateGas({from: walletAddress, value: price})
      console.log("estimated gas",gasAmount)

      console.log({from: walletAddress, value: price})

      bananaContract.methods
            .redeemRole(how_many_bananas)
  ...
  return (
    <div id="bodyy" className="flex flex-col items-center justify-center min-h-screen py-2">
      <Head>
        ...
        <meta property="og:url" content="https://movr-loot.org/" key="ogurl"/>
        <meta property="og:image" content="https://movr-loot.org/images/Hola.gif" key="ogimage"/>
        <meta property="og:site_name" content="https://movr-loot.org/" key="ogsitename" />

        <meta name="twitter:card" content="summary_large_image" key="twcard"/>
        <meta property="twitter:domain" content="movr-loot.org" key="twdomain" />
        <meta property="twitter:url" content="https://movr-loot.org/" key="twurl" />
        <meta name="twitter:title" content="MovrRole" key="twtitle" />
        <meta name="twitter:description" content="MovrRole" key="twdesc" />
        <meta name="twitter:image" content="https://movr-loot.org/images/Hola.gif" key="twimage" />
      </Head>
        ...
        <div className="md:w-2/3 w-4/5">
          ...
          <div className="mt-6 border-b-2 py-6">
            ...
            <div className="flex flex-col items-center">
                ...
                <span className="flex Poppitandfinchsans text-5xl text-white items-center bg-grey-lighter rounded rounded-r-none my-4 ">TOTAL ROLE MINTED:  <span className="text-blau text-6xl"> {!signedIn ?  <>-</>  :  <>{totalSupply}</> } / 512</span></span>
                ...
                <div id="mint" className="flex justify-around  mt-8 mx-6">
                  ...
                  <span className="flex Poppitandfinchsans text-5xl text-white items-center bg-grey-lighter rounded rounded-r-none px-3 font-bold">ROLES!</span>
    
                </div>
                {saleStarted ? 
                <button onClick={() => mintBanana(how_many_bananas)} className="mt-4 Poppitandfinchsans text-4xl border-6 bg-blau  text-white hover:text-black p-2 ">MINT {how_many_bananas} ROLE for {(bananaPrice * how_many_bananas) / (10 ** 18)} MOVR + GAS</button>        
                ...
                {saleStarted ? 
                <button onClick={() => claimWiseToken(how_many_bananas)} className="mt-4 Poppitandfinchsans text-4xl border-6 bg-blau  text-white hover:text-black p-2 ">REDEEM WISEMOVR #{how_many_bananas} FOR ROLE</button>        
                  : <button className="mt-4 Poppitandfinchsans text-4xl border-6 bg-blau  text-white hover:text-black p-2 ">SALE IS NOT ACTIVE OR NO WALLET IS CONNECTED</button>        
